...
const client = new Client(
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhcHBfaWQiOiI3Mjg5NjU5MTQ0NzY4MDY2ODUiLCJpYXQiOjE1OTM4Njk0MTd9._E6dCtkLswWgDySTPihh32Al9tvHPxFuxqY_eBk8waQ",
);
...
const doc = new GoogleSpreadsheet(
  "1Mci7KdAbGP3s_VWzzTCvLT3wEDN7XbO_zoRrHr0_Lfw",
);
...
async function updateProvinceIncome() {
  doc.loadInfo().then(() => {
    ...
    sheet.loadCells("A1:U27").then(() => {
      for (let i = 2; i <= 27; i++) {
        db.run(
          `UPDATE Province SET income = ${
            sheet.getCellByA1("S" + i).formattedValue
          } WHERE prov_id = ${i - 1}`,
        );
      }
    });
    ...
    sheetPop.loadCells("AN4:AO29").then(() => {
      for (let i = 4; i <= 29; i++) {
        db.run(
          `UPDATE Province SET population = '${
            sheetPop.getCellByA1("AN" + i).formattedValue
          }' WHERE prov_id = '${i - 3}'`,
        );
      }
      ...
      for (let i = 4; i <= 28; i++) {
        db.run(
          `UPDATE Province SET population_growth = '${
            sheetPop.getCellByA1("AO" + i).formattedValue
          }' WHERE prov_id = ${i - 3}`,
        );
      }
    });
...
function updateEmbedMessage(message) {
  ...
  embed.setFooter({
    text: "discussions powered by our most humble Imperator",
    iconURL:
  ...
  embed.setAuthor({
    ...
    url: "https://cdn.glitch.com/24cdd29f-170e-4ac8-9dc2-8abc1cbbaeaa%2Fimageedit_1_3956664875.png?v=1588186424473",
  });
  ...
  db.all(sql2, [], (err, rows) => {
    ...
    if (msgs.length <= 25) {
      ...
      secEmb.setColor("DarkRed");
      secEmb.setFooter({
        text: "Senate Meeting discussions powered by our most humble Imperator",
        iconURL:
          "https://cdn.discordapp.com/attachments/548918811391295489/776740280266784778/purpleDR.png",
      });
      secEmb.setThumbnail(
      ...
      args2.forEach((msg) =>
        secEmb.addFields({
          name: "motion",
          value: msg + "/" + parseInt(args2.indexOf(msg) + msgs.length, 10),
          inline: false,
        }),
      );
      msgs.forEach((msg2) =>
...
function postMussoFact() {
  ...
  embed.setFooter({
    text: "Facts powered by our most humble Imperator",
    iconURL:
      "https://cdn.discordapp.com/attachments/548918811391295489/776740280266784778/purpleDR.png",
  });

  embed.addFields({ name: "Fact:", value: randomElement });
  clientdc.channels.cache.get("514135876909924354").send({ embeds: [embed] });
}
...
function updateProvinceMessage() {
  ...
  embed.setColor("DarkRed");
  embed.setFooter({
    text: "Provinces powered by our most humble Imperator",
    iconURL:
      "https://cdn.discordapp.com/attachments/548918811391295489/776740280266784778/purpleDR.png",
  });
  embed.setThumbnail(
  ...
  db.all(sql2, [], (err, rows) => {
    ...
    if (msgs.length <= 25) {
      msgs.forEach((msg) =>
        embed.addFields({
          name: rows[msgs.indexOf(msg)].province,
          value: msg + "\n" + msgs.indexOf(msg),
        }),
      );
      var secEmb = new Discord.EmbedBuilder();

      clientdc.guilds.cache
        .get(GuildID)
        .channels.cache.get("817410470700515338")
      ...
      args2.forEach((msg) =>
        secEmb.addFields({
          name: "Provinces",
          value: msg + "\n" + parseInt(args2.indexOf(msg) + msgs.length, 10),
          inline: false,
        }),
      );
      msgs.forEach((msg2) =>
...
function updateRPDate(RPName) {
  let date = db2
    .prepare("SELECT * FROM Currentdates WHERE RP_Server = ?")
    .get(RPName);
  date.year++;

  clientdc.channels.cache.get("960998378265780281").send(date.year);

  db2
    .prepare("UPDATE Currentdates SET year = year + 1 WHERE RP_Server = ?")
    .run(RPName);
}
...
function registerSlashCommands() {
  ...
  let registerGovernor = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("name").setDescription("Discord handle").setRequired(true),
    );
  let appointProvince = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("governor")
        .setDescription("Name of the Governor given")
        .setRequired(true),
    )
    .addStringOption((option) =>
      option
        .setName("province")
        .setDescription("Name of the Province given")
        .setRequired(true),
    );
  let smotion = new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option
        .setName("motionid")
        .setDescription("Corresponding ID of the motion")
        .setRequired(true)
        .setMinValue(0)
        .setMaxValue(50)
        .setRequired(true),
  ...
  let motion = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("motion")
        .setDescription("the proposed motion")
        .setRequired(true),
    );
  ...
  let registerCandidates = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("election")
        .setDescription("what type of election")
        .setRequired(true),
    );
  ...
  let showprovinces = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("province")
        .setDescription("Name of the Province")
        .setRequired(true),
    );
  ...
  let challenge = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("first")
        .setDescription("Name of the first Gladiator")
        .setRequired(true),
    )
    .addStringOption((option) =>
      option
        .setName("second")
        .setDescription("Name of the second Gladiator")
        .setRequired(true),
    );
  globalCommands.push(
    ban,
  ...
  let showCCC = new SlashCommandBuilder()
    ...
    .setDescription(
      "Displays a Paragraph of the Catechism of the Catholic Church",
    )
  ...
  CronJob.schedule(
    ...
    () => {
      clientdc.channels.cache
        .get("549645921487421495")
        .send("<@&577587377195974656>");
    },
    {
      scheduled: true,
...
function createProvinceEmbed(picURL, indexStart, indexEnd) {
  ...
  let statement = db2.prepare(
    "SELECT * FROM Province JOIN Governor ON Province.Governor = gov_ID",
  );
  let rows = statement.all();
  for (let i = indexStart; i < indexEnd; i++) {
    provinces.addFields({
      name: rows[i].province,
      value: "<@" + rows[i].DiscordID + ">",
    });
  }
  return provinces;
}
...
clientdc.on("interactionCreate", async (interaction) => {
  switch (interaction.commandName) {
    ...
    case "mute":
      if (
        interaction.member.roles.cache.has("549712921450774556") ||
        interaction.member.roles.cache.has("546654987061821440")
      ) {
        const target = interaction.options.getMember("member");
        if (target) {
          let muteRole = clientdc.guilds.cache
            .get(interaction.guildId)
            .roles.cache.find((role) => role.name === "Imprisoned");
          ...
          interaction.options.getString("reason")
            ? emb.setDescription(
                `<@${member.user.id}> has been muted for ${ms(
      ...
      if (
        ...
        if (reason) {
          if (!member.roles.cache.has("546654987061821440")) {
            ...
            emb.setColor("#8f0713");
            emb.setDescription(
              `<@${member.user.id}> has been Executed in the name of the Imperator, Reason: ${reason}`,
            );
            emb.setTitle(
              "𝐈𝐌𝐏𝐄𝐑𝐀𝐓𝐎𝐑·𝐏𝐕𝐁𝐋𝐈𝐕𝐒",
              "https://cdn.glitch.com/24cdd29f-170e-4ac8-9dc2-8abc1cbbaeaa%2Fimageedit_1_3956664875.png?v=1588186424473",
            ...
            emb.setImage(
              "https://i.pinimg.com/originals/7f/03/a2/7f03a2a21b82d96679788401c3ef323f.jpg",
            );
          ...
          if (!member.roles.cache.has("546654987061821440")) {
            member
              .ban()
              .catch((err) => interaction.editReply("Cannot Ban this user"));
            let emb = new Discord.EmbedBuilder();
            emb.setColor("#8f0713");
            emb.setDescription(
              `<@${member.user.id}> has been Executed in the name of the Imperator`,
            );
            emb.setTitle(
              "𝐈𝐌𝐏𝐄𝐑𝐀𝐓𝐎𝐑·𝐏𝐕𝐁𝐋𝐈𝐕𝐒",
              "https://cdn.glitch.com/24cdd29f-170e-4ac8-9dc2-8abc1cbbaeaa%2Fimageedit_1_3956664875.png?v=1588186424473",
    ...
    case "showprovince":
      clientdc.users.fetch(interaction.user.id).then((u) => {
        let provinceName = interaction.options
          .getString("province")
          .toLowerCase();
        ...
        embP.setThumbnail(
          "https://cdn.discordapp.com/attachments/548918811391295489/846080867138011156/unknown.png",
        );
        ...
        doc.loadInfo().then(() => {
          ...
          sheet.loadCells("A1:J27").then(() => {
            ...
            let rows = db2
              .prepare("SELECT * FROM Province ORDER BY income DESC")
              .all();
            let province = db2
              .prepare(
                "SELECT * FROM Province JOIN Governor ON province.Governor = Governor.gov_ID WHERE province = ?",
              )
              .get(provinceName);
            let resource = db2
              .prepare("SELECT * FROM Resources WHERE province = ?")
              .get(provinceName);
            ...
            if (province) {
              ...
              for (let i = 2; i <= 27; i++) {
                if (
                  sheet.getCellByA1("B" + i).formattedValue === provinceName
                ) {
                  provinceIndex = i;
                }
              }
              ...
              if (provinceIndex === -1) {
                interaction.reply(
                  "Error with Province sheet name (please contact a province admin)",
                );
              } else {
                for (let i = 0; i < 7; i++) {
                  let cell = sheet.getCellByA1(
                    String.fromCharCode(68 + i) + provinceIndex,
                  ).formattedValue;
                  if (cell) {
                ...
                let actualTax =
                  provinceOrder <= 5 ? 1 - taxRateTop5 : 1 - taxRate;
                embP.setDescription(
                  province.income +
                    " <:DNR:782312774083674163> daily (untaxed) || " +
                    Math.floor(province.income * actualTax) +
                    " <:DNR:782312774083674163> daily (taxed) \n 🧍 Population: " +
                    province.population +
                    "\n Population Growth: " +
                    province.population_growth,
                );
                province.map != null
                  ? embP.setImage(province.map)
                  : embP.setImage(
                      "https://cdn.discordapp.com/attachments/559418842430963723/775017785969475634/unknown.png",
                    );
      ...
      break;
    case "provinces":
      let provinces = createProvinceEmbed(
        "https://cdn.discordapp.com/attachments/771315528471806032/1127167638620086352/image.png",
        0,
        25,
      );
        ...
        if (target) {
          ...
          members
            ...
            .then((unban) => {
              ...
              emb.setColor("#8f0713");
              emb.setDescription(
                `<@${target.id}> has been pardoned in the name of the Imperator`,
              );
              emb.setTitle(
                "𝐈𝐌𝐏𝐄𝐑𝐀𝐓𝐎𝐑·𝐏𝐕𝐁𝐋𝐈𝐕𝐒",
                "https://cdn.glitch.com/24cdd29f-170e-4ac8-9dc2-8abc1cbbaeaa%2Fimageedit_1_3956664875.png?v=1588186424473",
    ...
    case "kick":
      if (
        ...
        if (interaction.options.getString("reason")) {
          member
            .kick(member, { reason: interaction.options.getString("reason") })
            .then((ban) => {
              EmbedBuilder.setColor("#8f0713");
              EmbedBuilder.setDescription(
                `<@${member.id}> has been sent to Germania to fend off Barbarians in the name of the Imperator`,
              );
              ...
              EmbedBuilder.setImage(
                "https://i.pinimg.com/originals/22/b7/c9/22b7c9ed82e411d5c66037e820427622.jpg",
              );
      ...
      if (
        ...
        db.all(sql2, [], (err, rows) => {
          ...
          if (msgs.length <= 25) {
            ...
            const args2 = [];
            const length =
              msgs.length % 2 === 0 ? msgs.length : msgs.length + 1;
            ...
            args2.forEach((msg) =>
              secEmb.addFields({
                name: "motion",
                value:
                  msg + "/" + parseInt(args2.indexOf(msg) + msgs.length, 10),
                inline: false,
              }),
            );
            msgs.forEach((msg2) =>
    ...
    case "smotion":
      ...
      db.all(sql2, [], (err, rows) => {
        ...
        rows.forEach((row) => {
          available = true;
          if (
            row.motion.includes(".png") ||
            row.motion.includes(".jpg") ||
            row.motion.includes(".jpeg")
          ) {
            if (!row.motion.startsWith("http")) {
              const args2 = row.motion.split("http");
              embed.addFields({
                name: "Motion in question",
                value: args2[0],
                inline: true,
              });
        ...
        if (!available) {
          ...
          db.all("SELECT * FROM Motions", [], (err, rows) => {
            ...
            if (!rows[motionId]) {
              ...
              if (
                mot[motionId].includes(".png") ||
                mot[motionId].includes(".jpg") ||
                mot[motionId].includes(".jpeg")
              ) {
                if (!mot[motionId].startsWith("http")) {
                  ...
                  embed.setAuthor({
                    name: "𝐈𝐌𝐏𝐄𝐑𝐀𝐓𝐎𝐑·𝐏𝐕𝐁𝐋𝐈𝐕𝐒",
                    iconURL: clientdc.users.cache
                      .get(creator[motionId[1]])
                      .avatarURL(),
                  });
                  interaction.reply({ embeds: [embed] });
                } else {
                ...
                if (mot.length >= 25) {
                  var args2 = [];
                  const length =
                    mot.length % 2 === 0 ? mot.length : mot.length + 1;
                  ...
                  if (mot[motionId]) {
                    ...
                    embed.addFields({
                      name: "Motion in question",
                      value: args2[motionId - mot.length],
                      inline: false,
                    });
                  } else {
                    embed.addFields({
                      name: "Motion in question",
                      value: "no motion with such ID",
                      inline: false,
                    });
                  }
                  let avatarURL;
    ...
    case "deletemotion":
      if (
        ...
        db.all(
          "DELETE FROM Motions WHERE mID =" + motiontodelete,
          [],
          (err, rows) => {
            if (err) {
    ...
    case "motion":
      if (
        ...
        const filter = (response) => {
          return answer.includes(response.content.toLowerCase());
        };
    ...
    case "challenge":
      ...
      glademb.setImage(
        "https://qph.fs.quoracdn.net/main-qimg-03cc5a7d16ec433984a39059446d5c4b",
      );
      ...
      interaction.reply({ embeds: [glademb] }).then((res) => {
        ...
        let int = setInterval(function (php1, php2) {
          ...
          if (hp1 === 0 || hp2 === 0) {
            ...
            emb3.addFields({
              name: "Winner",
              value: hp1 === 0 ? args[1] : args[0],
            });
            emb3.setImage(
              "https://www.thecolosseum.org/wp-content/uploads/2017/09/gladiator-painting-optimized-970x652.jpg",
            );
            emb3.setColor(0xffd700);
            interaction
              .editReply({ embeds: [emb3] })
    ...
    case "registergovernor":
      try {
        db2
          .prepare("INSERT INTO Governor(DiscordID,governor) VALUES(?,?)")
          .run(
            interaction.options.getString("discordid"),
    ...
    case "appointprovince":
      try {
        let govId = db2
          .prepare("SELECT gov_ID FROM Governor WHERE governor = ?")
          .get(interaction.options.getString("governor"));

        db2
          .prepare("UPDATE Province SET Governor = ? WHERE province = ?")
          .run(govId.gov_ID, interaction.options.getString("province"));

        interaction.reply(
          "Governor " +
            interaction.options.getString("governor") +
            " appointed for " +
            interaction.options.getString("province"),
        );
      } catch (e) {
        interaction.reply("Something went wrong.");
        console.log(e);
    ...
    case "showcatechism":
      ...
      const response = await fetch(
        `https://www.vatican.va/archive/ENG0015/__P${paragraph}.HTM`,
      );
      ...
      const CCCembed = new Discord.EmbedBuilder()
        ...
        .setColor("Gold")
        .setFooter({
          text: "Pontifex Bovio I",
          iconURL:
            "https://cdn.discordapp.com/attachments/548918811391295489/776740280266784778/purpleDR.png",
        })
        .setAuthor({
          name: "Catechismus Catholicae Ecclesiae",
          iconURL:
            "https://cdn.discordapp.com/attachments/704000578568716368/1135879059285479494/png-transparent-chi-rho-symbol-christianity-symbol-miscellaneous-angle-text-removebg-preview.png",
        })
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/704000578568716368/1135868008674381844/city-logo-flag-vatican-city-flag-of-vatican-city-flag-of-kazakhstan-2018-video-games-flags-of-the-world-png-clipart-removebg-preview.png",
        );
    ...
    case "registercandidates":
      ...
      const candidateButton = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("can")
          .setStyle("Primary")
          .setLabel("Candidate!"),
...
function provinceIncome() {
  db.all(
    "SELECT * FROM Province JOIN Governor ON Province.Governor = gov_ID ORDER BY income DESC",
    [],
    (err, rows) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (incomeUntaxed !== 0) {
          if (currentTaxrate !== 0) {
            client
              .editUserBalance(GuildID, "700662464856981564", {
                cash: 0,
                bank: incomeUntaxed * currentTaxrate,
              })
              .catch((error) => console.log(error));
          }
          totalIncomeAllProvincesBrutto += incomeUntaxed;
          incomeUntaxed *= 1 - currentTaxrate;
          incomeUntaxed = rows[i].Boosted
            ? incomeUntaxed + incomeUntaxed * 0.01
            : incomeUntaxed;
          console.log(rows[i].Admin);
          parseInt(rows[i].Admin) !== 0
            ? client
                .editUserBalance(GuildID, String(rows[i].Admin), {
                  cash: 0,
                  bank: incomeUntaxed * 0.3,
                })
                .catch((error) => console.log(error))
            : (totalIncomeAllProvincesNetto += incomeUntaxed);
          client
            .editUserBalance(GuildID, String(rows[i].DiscordID), {
              cash: 0,
              bank: incomeUntaxed,
            })
            .catch((error) => console.log(error));
        ...
        if (i < 25) {
          emb.addFields({
            name: "Income: " + rows[i].province,
            value: "Income in denarii: " + incomeUntaxed,
          });
        } else {
          emb2.addFields({
            name: "Income: " + rows[i].province,
      ...
      emb.setTitle(
        "total pre-tax: " +
          totalIncomeAllProvincesBrutto +
          "\n total post-tax: " +
          totalIncomeAllProvincesNetto,
      );
      clientdc.channels.cache
        .get("791427239601766482")
        .send({ embeds: [emb, emb2] });
...
function collectTaxes() {
  ...
  client.getGuildLeaderboard(GuildID).then((bal) => {
    ...
    for (let i = 0; i < mem; i++) {
      let user = clientdc.guilds.cache
        .get(GuildID)
        .members.cache.get(bal[i].user_id);
      ...
      if (user) {
        ...
        if (user.roles.cache.get("548455163044560897")) {
          ...
          if (bal[i].bank > bal[i].cash) {
            client.editUserBalance(
              GuildID,
              bal[i].user_id,
              { cash: 0, bank: -bal[i].total * 0.17 },
              "taxes",
            );
          } else {
            client.editUserBalance(
  ...
  if (button.isButton()) {
    if (provincebuttons.includes(button.customId)) {
      switch (button.customId) {
        case "fac":
          var imageUrl =
            "https://cdn.discordapp.com/attachments/543787157127561216/932970514866143262/unknown.png";
          break;
        case "pre":
          var imageUrl =
            "https://cdn.discordapp.com/attachments/771315528471806032/1127167638620086352/image.png";
        ...
        case "cul":
          var imageUrl =
            "https://cdn.discordapp.com/attachments/514135876909924354/813764720607625276/unknown.png";
          break;
        case "nex":
          var imageUrl =
            "https://cdn.discordapp.com/attachments/771315528471806032/1127167638620086352/image.png";
      ...
      let provinces;
      if (embedIndex) {
        provinces = createProvinceEmbed(
          imageUrl,
          embedIndex.indexStart,
          embedIndex.indexEnd,
        );
      } else {
      ...
      await button.reply({
        content: "Successfully registered as a candidate",
        ephemeral: true,
      });
    }
  }
...
clientdc.on("messageReactionRemove", (reaction, user) => {
  if (
    user.id === "220590173962895360" &&
    reaction.emoji.id === "640270832115122196"
  ) {
    let available = false;
    db.all(
      "SELECT * FROM Generals WHERE DiscordID = " + reaction.message.author,
      [],
      (err, rows) => {
        if (err) {
          throw err;
        ...
        if (available) {
          db.run(
            "UPDATE Generals SET generals = generals-1 WHERE DiscordID = " +
    ...
    db.all(
      ...
      (err, rows) => {
        ...
        if (available) {
          ...
          db.run(
            "INSERT INTO Generals(DiscordID,generals) VALUES('" +
              reaction.message.author.id +
              "',1)",
          );
        }
...
function addTransaction(amount, reason) {
  db.run(
    "INSERT INTO Treasury(amount,reason) VALUES(" +
      amount +
      ",'" +
      reason +
      "')",
...
clientdc.on("messageCreate", (message) => {
  ...
  if (
    message.content.toLowerCase().includes(command + "registercensoroffice")
  ) {
    if (
      message.member.roles.cache.has("549712921450774556") ||
      message.member.roles.cache.has("704023155487932457")
    ) {
      let args = message.content.split(" ");
      ...
      db.run(
        "UPDATE Province SET HasCensorOffice = 'true' WHERE province = '" +
          args[2] +
          "'",
      );
  ...
  if (message.content === command + "showDB") {
    if (message.author.id === "325296044739133450") {
      ...
      message.channel.send(
        "sorry but you're not the Imperator " + "<@325296044739133450>",
      );
    }
  }
    ...
    if (contenders.length === 6) {
      if (
        ...
        for (let i = 0; i < defenders.length; i++) {
          ...
          let queryLegion =
            "SELECT * FROM LegionComposition JOIN Legion,units ON L_ID = Legion AND U_ID = LegionComposition.unit WHERE name = '" +
            legionsA[0] +
            "' ";
          ...
          db.all(queryLegion, [], (error, attackingLegions) => {
            let queryNonLegions =
              "SELECT * FROM Factionunits JOIN Factions ON F_ID = faction WHERE Name = '" +
              nonLegionsA[0] +
              "' ";
            ...
            db.all(queryNonLegions, [], (error2, attackingFactions) => {
              ...
              db.all(queryLegionsD, [], (error2, defendingLegions) => {
                ...
                db.all(queryNonLegionsD, [], (error3, defendingFactions) => {
                  ...
                  let emb = new Discord.EmbedBuilder();
                  emb.setImage(
                    "https://1.bp.blogspot.com/-ZbMMWQO4toE/Vi1WyeZNqnI/AAAAAAAABKE/Htni5mZ--Xo/s1600/Roman%2BLegion.jpg",
                  );
                  let a = "Attackers: ";
                  ...
                  message.channel.send({ embeds: [emb] }).then((res) => {
                    ...
                    for (let i = 0; i < attackingLegions.length; i++) {
                      aLAttack +=
                        parseInt(attackingLegions[i].Attack) *
                        parseInt(percentageOfArmies[0]) *
                        (1 + parseInt(attackingLegions[i].attack_boni));
                      aLDefense +=
                        parseInt(attackingLegions[i].Defense) *
                        parseInt(percentageOfArmies[0]) *
                        (1 + parseInt(attackingLegions[i].defense_boni));
                      aLHealth +=
                        parseInt(attackingLegions[i].Health) *
                        parseInt(percentageOfArmies[0]);
                    ...
                    for (let i = 0; i < defendingLegions.length; i++) {
                      dLAttack +=
                        parseInt(
                          defendingLegions[i].Attack *
                            parseInt(percentageOfArmies[0]),
                        ) *
                        (1 + parseInt(defendingLegions[i].attack_boni));
                      dLDefense +=
                        parseInt(
                          defendingLegions[i].Defense *
                            parseInt(percentageOfArmies[0]),
                        ) *
                        (1 + parseInt(defendingLegions[i].attack_boni));
                      dLHealth += parseInt(
                        defendingLegions[i].Health *
                          parseInt(percentageOfArmies[0]),
                      );
                    ...
                    for (let i = 0; i < attackingFactions.length; i++) {
                      aFAttack +=
                        parseInt(attackingFactions[i].Attack) *
                        parseFloat(attackingFactions[i].percentage);
                      aFDefense +=
                        parseInt(attackingFactions[i].Defense) *
                        parseFloat(attackingFactions[i].percentage);
                      aFHealth +=
                        parseInt(attackingFactions[i].Health) *
                        parseFloat(attackingFactions[i].percentage);
                    ...
                    for (let i = 0; i < defendingFactions.length; i++) {
                      dFAttack +=
                        parseInt(defendingFactions[i].Attack) *
                        parseFloat(defendingFactions[i].percentage);
                      dFDefense +=
                        parseInt(defendingFactions[i].Defense) *
                        parseFloat(defendingFactions[i].percentage);
                      dFHealth +=
                        parseInt(defendingFactions[i].Health) *
                        parseFloat(defendingFactions[i].percentage);
                    ...
                    let int = setInterval(function () {
                      if (cturn === true) {
                        ...
                        emb2.emb2.setColor("#8B0000");
                        res.edit(emb2);
                        cturn = false;
                      } else {
                        hp2 -= Math.abs(
                          Math.floor(
                            Math.random() * (aFAttack + aLAttack) -
                              (dLDefense + dFDefense),
                          ),
                        );
                        ...
                        let emb2 = new Discord.EmbedBuilder();
                        emb2.setImage(
                          "https://i.pinimg.com/originals/fa/94/71/fa94710c7832f410703bf600c903de9b.jpg",
                        );
                        emb2.addFields({ name: a, value: hp1, inline: true });
                        emb2.addFields({ name: d, value: hp2, inline: true });
                        emb2.emb2.setColor("#8B0000");
                      ...
                      if (hp1 === 0 || hp2 === 0) {
                        ...
                        emb3.setImage(
                          "https://digitalmapsoftheancientworld.files.wordpress.com/2019/02/2118.png",
                        );
                        emb3.emb3.setColor("#FFDF00");
  ...
  if (message.content.toLowerCase() === command + "showlegions") {
    ...
    db.all(
      "SELECT * FROM Legion JOIN Province ON Legion.province = Province.prov_id",
      [],
      (err, rows) => {
  ...
  if (message.content.toLowerCase() === command + "provinces") {
    let emb = createProvinceEmbed(
      "https://cdn.discordapp.com/attachments/543787157127561216/932676494466113626/unknown.png",
      0,
      25,
    );
    ...
    message.channel.send({ embeds: [emb] }).then((m) => {
      ...
      let pics = [
        "https://cdn.discordapp.com/attachments/548918811391295489/846080867138011156/unknown.png",
        "https://cdn.discordapp.com/attachments/771315528471806032/845960254520557598/unknown.png",
        "https://cdn.discordapp.com/attachments/514135876909924354/813764720607625276/unknown.png",
      ];
  ...
  if (message.content.toLowerCase().includes(command + "showprovince")) {
    ...
    let emb = new Discord.EmbedBuilder();
    emb.setThumbnail(
      "https://cdn.discordapp.com/attachments/680733248396984330/785230951998947328/unknown.png",
    );
    emb.setColor("#cd1121");
    let word;
    ...
    db.all("SELECT * FROM Province ORDER BY income DESC", [], (err, order) => {
      db.all(
        "SELECT resource, population, income, map, population_growth, DiscordID, province.province AS pro FROM Province JOIN Resources,Governor ON prov_id =    Resources.province AND Governor.gov_ID = Province.Governor WHERE Province.province = '" +
          province +
          "'",
        [],
        (err, rows) => {
            ...
            clientdc.guilds.cache
              ...
              .then((u) => {
                ...
                emb.setDescription(
                  rows[0].income +
                    " <:DNR:782312774083674163> daily (untaxed) || " +
                    Math.floor(rows[0].income * actualTax) +
                    " <:DNR:782312774083674163> daily (taxed) \n 🧍 Population: " +
                    rows[0].population +
                    "\n" +
                    " Population Growth: " +
                    rows[0].population_growth,
                );
                rows[0].map != null
                  ? emb.setImage(rows[0].map)
                  : emb.setImage(
                      "https://cdn.discordapp.com/attachments/559418842430963723/775017785969475634/unknown.png",
                    );
                message.channel.send({ embeds: [emb] });
  ...
  if (message.content.toLowerCase() === command + "lb") {
    ...
    emb.setColor("#e94606");
    emb.setAuthor({
      name: "Leaderboard for the most Generalissimo reactions",
    ...
    db.all("SELECT * FROM Generals ORDER BY generals DESC", [], (err, rows) => {
      ...
      rows.forEach((row) => {
        if (
          row.DiscordID !== "220590173962895360" &&
          clientdc.users.cache.get(row.DiscordID)
        ) {
          emb.addFields({
            name: clientdc.guilds.cache
              .get("514135876909924352")
              .members.cache.get(row.DiscordID).displayName,
            value:
              row.generals +
              "\n" +
  ...
  if (message.content.toLowerCase() === command + "general") {
    ...
    db.get(
      "SELECT * FROM Generals WHERE DiscordID = " + message.author.id,
      [],
      (err, row) => {
        if (err) {
          throw err;
        ...
        if (row === undefined) {
          emb.addFields({ name: "Generals", value: "0", inline: true });
        } else {
          emb.addFields({
            name: "Generals",
            value: row.generals,
          ...
          emb.setAuthor({
            ...
            iconURL: clientdc.users.cache.get(message.author.id).avatarURL(),
          });
        }
  ...
  if (message.content.toLowerCase().includes(command + "insertcandidate")) {
    ...
    db.get(
      "SELECT cID FROM candidates WHERE DiscordID = '" + args[1] + "';",
      [],
      (err, rowss) => {
        if (rowss === undefined) {
        ...
        cID = rowss.cID;
        db.get(
          "SELECT eID FROM Elections WHERE Month = '" +
            args[3] +
            "' AND Title = '" +
            args[2] +
            "'",
          ...
          (err, row) => {
            eID = row.eID;
            db.run(
              'INSERT INTO CandidateElections(candidate,Election,number) VALUES("' +
                cID +
                '","' +
                eID +
                '",' +
  ...
  if (message.content === "ping") {
    ...
    if (
      clientdc.guilds.cache
        ...
        .roles.cache.get("548455163044560897")
    ) {
  ...
  if (message.content.toLowerCase().includes(command + "bet")) {
    let args = message.content.split(" ");
    if (
      (args.length === 3 && args[1] !== args[2]) ||
      (args.length === 2 && args[1] !== message.author.username)
    ) {
      if (!BattleAlreadyCommenced) {
            ...
            if (
              ...
              if (bet <= 10000 && bet > 0 && bet !== 0) {
                if (c.cash >= bet) {
                  ...
                  message.channel.send(
                    "invalid bet not enough cash, you currently have " +
                      c.cash +
                      " <:DNR:782312774083674163>",
                  );
                }
              } else {
                message.channel.send(
                  "invalid bet: max bet of 10000 <:DNR:782312774083674163>",
                );
              }
            } else {
        ...
        collector.on("end", (collected) => {
          ...
          if (args.length === 2) {
            ...
            message.channel.send({ embeds: [emb] }).then((res) => {
              ...
              let int = setInterval(function (php1, php2) {
                ...
                if (hp1 === 0 || hp2 === 0) {
                  ...
                  emb3.addFields({
                    name: "Winner",
                    value: hp1 === 0 ? args[1] : message.author.username,
                  });
                  emb3.setImage(
                    "https://www.history.com/.image/c_fill%2Ccs_srgb%2Cfl_progressive%2Ch_400%2Cq_auto:good%2Cw_620/MTU3ODc4NjAzNTMwOTA1MzEx/list-10-things-you-may-not-know-about-gladiators-2.jpg",
                  );
                  emb3.setColor(0xffd700);
            ...
            message.channel.send("invalid parameters given");
          }
        });
      } else {
        message.channel.send(
          "A battle is currently occuring, concurrent battles are not allowed!",
        );
      }
    } else {
  ...
  if (
    message.content.toLowerCase().includes(command + "deleteallmotions") ||
    (message.content.toLowerCase().includes("ave imperator publius") &&
      message.channel.id === 549645921487421495)
  ) {
    if (
      ...
      db.all("DELETE FROM Motions", [], (err, rows) => {
        if (err) {
          ...
          message.channel.send(
            "Ave Imperator Publius Augustus -motions deleted",
          );
  ...
  if (!message.author.bot && message.author.id !== "241349696856129539") {
    ...
    if (
      (message.content.toLowerCase().includes("the general") &&
        message.channel.id === "543787717725519892") ||
    ...
    if (
      (message.content.toLowerCase().includes("imperator") &&
        message.channel.id === "660253007916826625") ||
    ...
    if (message.content.toLowerCase().includes(command + "russia")) {
      ...
      if (args[1].includes("+")) {
        ...
        message.channel.send(
          parseInt(
            Math.random() * prob[0] + parseInt(prob[1]),
          ).toLocaleString(),
        );
      } else {
        message.channel.send(
          parseInt(Math.random() * args[1]).toLocaleString(),
        );
      }
    }
    if (
      message.content.toLowerCase().startsWith("!") &&
      message.channel.id === "630588104184430643"
    ) {
    ...
    if (
      !message.content.toLowerCase().startsWith("!") &&
      message.channel.id === "819602635966513172"
    ) {
